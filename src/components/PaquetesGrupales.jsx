...
export function PaquetesGrupales({ user, profile, onOpenAuth }) {
  ...
  const handleUnirmeGrupo = async (paquete) => {
    if (!user) {
      alert('Para unirte a un grupo debes iniciar sesión o registrarte primero.');
      onOpenAuth();
      return;
    }

    const confirmacion = window.confirm(`¿Confirmas tu lugar en "${paquete.titulo}" por S/ ${paquete.precio_persona.toFixed(2)}?`);
    ...
    try {
      ...
      const response = await fetch('/php/api/inscribirse_grupo.php', {
        ...
        body: JSON.stringify({
          ...
          user_nombre: profile ? profile.nombre_completo : 'Viajero',
      ...
      if (data.success) {
        alert('🎉 ¡Felicitaciones! Te has unido exitosamente al grupo. Se ha enviado la confirmación a tu correo.');
        cargarPaquetes();
      } else {
        alert(`⚠️ ${data.message || 'No se pudo completar la inscripción.'}`);
      }
    } catch (err) {
